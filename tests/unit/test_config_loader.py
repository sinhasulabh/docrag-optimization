...
def test_load_baseline_dense_full():
    spec = load_experiment_config(CONFIGS_DIR / "baseline_dense_full.yaml")
    assert spec.config.name == "baseline_dense_full"
    assert len(spec.doc_names) == 288
    assert spec.config.retrieval.mode == "dense"
    assert spec.config.retrieval.index_type == "flat"


def test_load_hybrid_rerank_full():
    spec = load_experiment_config(CONFIGS_DIR / "hybrid_rerank_full.yaml")
...
def test_the_two_example_configs_share_offline_fingerprint():
    baseline = load_experiment_config(CONFIGS_DIR / "baseline_dense_full.yaml")
    hybrid = load_experiment_config(CONFIGS_DIR / "hybrid_rerank_full.yaml")
